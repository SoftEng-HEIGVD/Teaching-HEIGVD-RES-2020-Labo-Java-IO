...
public class Application implements IApplication
{
	...
	public static void main(String[] args)
	{
		...
		try {
			...
			System.err.println(
				"The command accepts a single numeric argument (number of quotes to fetch)");
			System.exit(-1);
		}
		...
		try {
			 ...
			 */
			Writer writer = new StringWriter(); // we create a special writer that will
			// send characters into a string (memory)
			app.printFileNames(writer);   // we hand over this writer
			// to the printFileNames method
	...
	public void fetchAndStoreQuotes(int numberOfQuotes) throws IOException
	{
		...
		for (int i = 0; i < numberOfQuotes; i++) {
			 ...
			 * As you can see, this method handles the first part of the lab. It uses the
			 * web service
			 * client to fetch quotes. We have removed a single line from this method. It
			 * is a call to
			 * one method provided by this class, which is responsible for storing the
			 * content of the
	...
	/**
	 * This method uses a IFileExplorer to explore the file system and prints the name of
	 * each
	...
	void printFileNames(final Writer writer)
	{
		...
		explorer.explore(new File(WORKSPACE_DIRECTORY), new IFileVisitor()
		{
			...
			{
				/*
				 * There is a missing piece here. Notice how we use an anonymous class here.
				 *  We provide the implementation
				 * of the the IFileVisitor interface inline. You just have to add the body
				 * of the visit method, which should
				 * be pretty easy (we want to write the filename, including the path, to the
				 *  writer passed in argument).
				 */
				try {
					writer.append(file.getPath()).append(System.lineSeparator());
				} catch (IOException e) {}
			}
		});
	}
