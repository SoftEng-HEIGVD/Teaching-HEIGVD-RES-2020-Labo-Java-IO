...
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
    ...
    stringBuilder.append(WORKSPACE_DIRECTORY).append(File.separator);

    for (String tag : quote.getTags()) {
      stringBuilder.append(tag).append(File.separator);
    }
    new File(stringBuilder.toString()).mkdirs();

    OutputStreamWriter oswriter = new OutputStreamWriter(new FileOutputStream(stringBuilder.toString() + filename + ".utf8"), "UTF-8");
