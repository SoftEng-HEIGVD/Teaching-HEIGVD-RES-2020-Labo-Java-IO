...
/**
 *
 * @author Olivier Liechti, Diluckshan
 */
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
    // Path with Directory tag
    String path = WORKSPACE_DIRECTORY;

    // All tags are added to the path
    for (String tag : quote.getTags()) {
    ...
    try {
      ...
      String pathWithQuote = path + File.separator + filename;

      //Creation Directories and sub directories
      File directories = new File(path);
      directories.mkdirs();

      // Outputstream writer is opened to write the file
      OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(pathWithQuote), "UTF-8");
      writer.write(quote.getQuote());
      writer.flush();
      writer.close();

    }catch (IOException except) {
      LOG.log(Level.SEVERE, "Unable to store in the local system ", except.getMessage());
    }
    ...
    explorer.explore(new File(WORKSPACE_DIRECTORY), new IFileVisitor() {
      ...
      public void visit(File file) {
       ...
       try{
         ...
         LOG.log(Level.SEVERE,"Erreur : no quotes " , except.getMessage());
       }
