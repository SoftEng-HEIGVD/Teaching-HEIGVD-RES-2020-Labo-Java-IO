...
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
    ...
    String pathToStore = WORKSPACE_DIRECTORY;

    new File(pathToStore).mkdir();

    for(String tag : quote.getTags()){
      new File(String.format("%s/%s", pathToStore, tag)).mkdir();
      pathToStore += "/" + tag;
    }

    File inputFile = new File(String.format("%s/%s", pathToStore, filename));
